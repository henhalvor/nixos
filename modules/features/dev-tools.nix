      ...
      customCommands:
        ...
        - key: "C"
          ...
          command: |
            ...
            ${pkgs.mise}/bin/mise exec -- opencode run -m github-copilot/gpt-5-mini --format json "Generate a concise git commit message for these changes. Output ONLY the commit message text with no markdown, code blocks, or explanations. Use conventional commit format:\n\n$DIFF" | jq -r 'select(.type == "text") | .part.text' | paste -sd ' ' - > /tmp/commit_msg && GIT_EDITOR=vim git commit -e -F /tmp/commit_msg
          output: terminal
    '';
