...
{...}: {
  flake.nixosModules.hpResticS3 = {
    ...
    identityExport = pkgs.writeShellApplication {
      ...
      runtimeInputs = with pkgs; [coreutils jq openldap postgresql util-linux];
      text = ''
        set -o pipefail
    ...
    vaultStage = pkgs.writeShellApplication {
      ...
      runtimeInputs = with pkgs; [coreutils curl jq rsync util-linux];
      text = ''
        set -o pipefail
    ...
    hermesExport = pkgs.writeShellApplication {
      ...
      text = ''
        ...
        [[ -n ${lib.escapeShellArg (cfg.hermesExportCommand or "")} ]] || fail "no reviewed Hermes-native export command is configured"
        exec 9>${sourceLock}; flock -x 9 || fail "could not acquire source lock"
        candidate="$(mktemp -d "$root/candidate.XXXXXX")"
        if ! HERMES_EXPORT_DEST="$candidate" ${pkgs.bash}/bin/bash -c ${lib.escapeShellArg (cfg.hermesExportCommand or "")}; then
    ...
    resticRun = pkgs.writeShellApplication {
      ...
      runtimeInputs = with pkgs; [coreutils restic util-linux];
      text = ''
  ...
  in {
    ...
    config = lib.mkIf cfg.enable {
      assertions = [
        { assertion = config.my.opencloud.enable; message = "my.hpBackup requires my.opencloud.enable."; }
        { assertion = config.my.opencloudConsistentSource.enable; message = "my.hpBackup requires my.opencloudConsistentSource.enable."; }
        { assertion = config.my.githubMirror.enable; message = "my.hpBackup requires my.githubMirror.enable."; }
      ];
      ...
      sops.secrets.RESTIC_AWS_ACCESS_KEY_ID = { sopsFile = ../../../secrets/hp-backup.yaml; mode = "0400"; };
      sops.secrets.RESTIC_AWS_SECRET_ACCESS_KEY = { sopsFile = ../../../secrets/hp-backup.yaml; mode = "0400"; };
      sops.secrets.RESTIC_AWS_DEFAULT_REGION = { sopsFile = ../../../secrets/hp-backup.yaml; mode = "0400"; };
      sops.secrets.RESTIC_REPOSITORY = { sopsFile = ../../../secrets/hp-backup.yaml; mode = "0400"; };
      ...
      systemd.tmpfiles.settings."10-hp-backup" = {
        "${statusDir}".d = { mode = "0700"; user = "root"; group = "root"; };
        "/var/lib/opencloud-identity-backup".d = { mode = "0700"; user = "root"; group = "root"; };
        "/var/lib/opencloud-identity-backup/latest".d = { mode = "0700"; user = "root"; group = "root"; };
        "/var/lib/vault-backup".d = { mode = "0700"; user = "root"; group = "root"; };
        "/var/lib/vault-backup/latest".d = { mode = "0700"; user = "root"; group = "root"; };
        "/var/lib/hermes-backup".d = { mode = "0700"; user = "root"; group = "root"; };
        "/var/lib/hermes-backup/latest".d = { mode = "0700"; user = "root"; group = "root"; };
      };
