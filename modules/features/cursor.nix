# Cursor — AI code editor (currently unused)
# Source: home/modules/applications/cursor.nix
# Template B2: HM-only
{ self, ... }: {
  ...
  flake.homeModules.cursor = { config, pkgs, lib, pkgs-unstable, ... }: {
    # Make sure vscode-neovim (asvetliakov.vscode-neovim) is installed

    # Install the Cursor package
    home.packages = with pkgs; [ pkgs-unstable.code-cursor ];
