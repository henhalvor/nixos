# Rofi — application launcher with glass theme
# Source: home/modules/desktop/launchers/rofi.nix + rofi-theme.nix
# Template B2: HM-only with Stylix colors, wallpaper picker, power desktop entries
{self, ...}: {
  flake.nixosModules.rofi = {lib, ...}: {
  ...
  in {
    home.packages = with pkgs; [rofi wl-clipboard libnotify];
    ...
    home.file = {
      ...
      ".config/rofi/themes/glass.rasi".text = ''
        * {
        ...
        element normal.normal  { background-color: transparent; text-color: #${colors.base05}; }
        element normal.active  { background-color: transparent; text-color: #${colors.base0B}; }
        element normal.urgent  { background-color: transparent; text-color: #${colors.base08}; }

        element selected.normal { background-color: rgba(${colors.base05-rgb-r}, ${colors.base05-rgb-g}, ${colors.base05-rgb-b}, 0.1); text-color: #${colors.base0D}; }
        element selected.active { background-color: rgba(${colors.base05-rgb-r}, ${colors.base05-rgb-g}, ${colors.base05-rgb-b}, 0.1); text-color: #${colors.base0B}; }
        element selected.urgent { background-color: rgba(${colors.base08-rgb-r}, ${colors.base08-rgb-g}, ${colors.base08-rgb-b}, 0.2); text-color: #${colors.base08}; }

        element alternate.normal { background-color: transparent; text-color: #${colors.base05}; }
        element alternate.active { background-color: transparent; text-color: #${colors.base0B}; }
        element alternate.urgent { background-color: transparent; text-color: #${colors.base08}; }

        scrollbar { width: 4px; border: 0px; handle-width: 8px; padding: 0px; background-color: transparent; handle-color: #${colors.base05}; }
        message { border: 0px; padding: 0px; background-color: transparent; }
        textbox { text-color: #${colors.base05}; background-color: transparent; }
      '';
          ...
          CHOICE=$(
            find "$WALLPAPER_DIR" -maxdepth 1 -type f \
              ...
              rofi -dmenu -i -show-icons \
                ...
                -theme-str '
                window { width: 58%; }
                listview { layout: vertical; columns: 3; fixed-columns: true; lines: 3; fixed-height: false; dynamic: true; }
                element { orientation: vertical; }
                element-icon { size: 200px; horizontal-align: 0.5; margin: 0px 0px 2px 0px; }
      ...
      # Power desktop entries for rofi
